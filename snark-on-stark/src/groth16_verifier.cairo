...
pub trait IGroth16VerifierBLS12_381<TContractState> {
    fn verify_groth16_proof_bls12_381(
        self: @TContractState, full_proof_with_hints: Span<felt252>, zktp_add : ContractAddress,
    ) -> Option<Span<u256>>;
}
...
pub trait IGetTicket<TContractState> {
    fn invalidate_ticket(ref self: TContractState, nullifierhash: u256, commitmenthash: u256);
}
...
mod Groth16VerifierBLS12_381 {
    ...
    use super::IGetTicketDispatcher;
    use super::IGetTicketDispatcherTrait;
    ...
    impl IGroth16VerifierBLS12_381 of super::IGroth16VerifierBLS12_381<ContractState> {
        fn verify_groth16_proof_bls12_381(
            self: @ContractState, full_proof_with_hints: Span<felt252>, zktp_add : ContractAddress,
        ) -> Option<Span<u256>> {
            ...
            if check == true {
                //todo call the external function for futere
                let account = IGetTicketDispatcher {
                    contract_address: zktp_add,
                };

                account
                    .invalidate_ticket(
                        *groth16_proof.public_inputs.at(0), *groth16_proof.public_inputs.at(1),
                    );
